...
* For the `depositFunds` method that accepts a cash amount, we'd like the ability to deposit funds in a variety of currencies. Add a parameter to the method that accepts a currency type, and develop a new object that encapsulates the currency converter logic for converting a cash amount to USD.
